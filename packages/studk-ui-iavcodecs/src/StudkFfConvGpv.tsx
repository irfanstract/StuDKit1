...
interface WithIDAndFfEngine {
  readonly id1: string | number,
  readonly coreEngn: FFmpeg,
}

const useFfAvConvGpv = (
  ...
  function (...[coreEngn] : [coreEngn: FFmpeg] )
  {
    ...
    const errorsInfo = (
      React.useMemo(() => (
        (s && (!s.done && (s.error && ({ error: s.error, errorMsg: String(s.error), } as const) ) ) ) || { error: null, errorMsg: null }
      ) , [
        s ,
      ])
    ) ;
    ...
    const resetToCustomRendering = (
      StudkReactJsOvcUtil.useRefreshedCallback(function <const XPostDat extends object | string> (...[asyncMainRun, { doClosingReadbackAsync, }, ]: (
        ArgsWithOptions<[
          asyncMainRun: ((opts: WithIDAndFfEngine ) => Promise<XPostDat> ) ,
        ] , {
          //
          doClosingReadbackAsync: ((opts: NoInfer<WithIDAndFfEngine & { dat: XPostDat }>) => Promise<Blob >) ,
        }>
      ) ) {
        ...
        const r0 = (
          ...
          (async () => {
            ...
            ;
            const postprocDat = (
              await asyncMainRun({ id1, coreEngn, })
            ) ;
            console["log"](`(taskid=${id1 }) successful conv` ) ;

            const fOAsBlob = (
              await doClosingReadbackAsync({ id1, coreEngn, dat: postprocDat, })
            ) ;
            const fO = new Uint8Array(await fOAsBlob.arrayBuffer() ) ;
            console["log"](`(taskid=${id1 }) successful readback` ) ;

            if (typeof fO === "string") { return util.throwTypeError() ; }
            const fOAsUrl = (
              toBlockDataUrl({
                contentType: fOAsBlob.type ,
                dataInBytes: fO ,
              })
            ) ;
    ...
    return {
      ...
      resetToCustomRendering ,

    } as const ;
  }
) ;

const useFfAvConvGpvExt = (
  ...
  function (...[coreEngn] : [coreEngn: FFmpeg] )
  {
    ...
    const resetToLavfiRendering = (
      ...
      React.useCallback((...[mainCode = `testsrc2=duration=5.1` ]: [code: string] | [code ?: never]) => {
        ...
        return (
          resetToCustomRendering(async ({ id1, coreEngn, }) => {
            ;
            const fnm = "main" + ("." + fmt) ;
            ...
            doClosingReadbackAsync: (
              async ({ id1, coreEngn, dat: { fnm, } , }) => {
                const bytes = (
                  await coreEngn.readFile(fnm, )
                );
                return (
                  new Blob([bytes] , {
                    //
