...
import {
  XCodeC ,
} from "studk-ui-encore/src/AsciiBlockUi/CodeBlockEditorC" ;
...
import {
  TceAdvC ,
} from "studk-ui-encore/src/RichTextComponents/TceAdvC" ;
        ...
        <div >
          { (
            ...
            .map((_, i) => StudkReactJs.withExtraSemanticProperties({
              ...
              valueAsHtml={cInHtml }
              editable
              onChange={({ newValueInHtml, existingValueInHtml: expectedExistingValAsHtml, }) => {

                void (
                  // true ||
                  setCInHtml(actualExistingValAsHtml => {
  
                    Cd: {
  
                      if ((expectedExistingValAsHtml === actualExistingValAsHtml) ) {
                        ;

                        ;
                        if (newValueInHtml === expectedExistingValAsHtml ) {
                          ;

                          void (
                            console["info"](`zero net change; discarding change.`, {
                        ...
                        void (
                          console["warn"](`race condition; discarding change.`, )
                          ,
                    ...
                    return actualExistingValAsHtml ;
                  } )
                ) ;
...
// const XCodeC = (
//   (ReactJsBasedCustomIntrinsicElement.definePrivatelyWithRenderFnAndProgrammaticItcBaseClassAlt )("studk-asciicodeblockalt" , ({ value: cInHtml, onChange, }: (
